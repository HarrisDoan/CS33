 ...
 *   Rating: 3
 */
// First we can determine the sign of x and y. If the sign of x is 0 and the sign of y is 1 than we clearly know x is larger. A positive value will always be greater than a negative value. We can shift the top bit of the binary to figure out whether or not its 1 or 0. If they're both a positive or both a negative, we then need to compare the difference between the two values. The larger value subtracted from the smaller value will result in a positive. The result will be the comparison of x and y's sign bit since if one is negative and one is positive its an easy picking, while if its not so obvious than it be the difference of the two in which the greater is returned. However since we are dealing with Two's, we also need to account of any possible overflow.
...
int twosComp2SignMag(int x) {
  int sign_mask; //Utilize a sign masking
  int x_signbit; //Obtain the sign bit to determine the sign of the value
  int abs_x; // Absolute value of X => magnitude of x -> compared to the signed value
  int result;
  sign_mask = x >> 31;
 ...
 */
int isPower2(int x) {
  int signbit; //Check the sign bit of x
  int zero_check; //Check if x is 0 or not, using logical NOT operator
  int pw2; //Check if its a power of 2 -> Formula: x & (x-1) = 0
  int result;
